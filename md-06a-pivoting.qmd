...
2.  What's the `tidyverse` Package? Describe in maximum two sentences below.
    ...
    -\>the tidyverse Package is a collection of packages that are most often used for data analysis. so importing, manipulating, visualizing data and so forth.
...
1.  Starting with the `waste_data_long_mean` object, execute the code and observe the output in the Console. Would you publish this table in this format in a report?
    ...
    -\>no I wouldn't, because while good for computational data analysis, for readability it's lacking. on top of that I think it's more interesting to compare the different income categories on the waste categories, so group it by waste category.
...
2.  Execute the code and observe the output in the Console. Would you publish this table in a report in this format?
    ...
    -\>now it's in the format I described above, so this I would publish this table in a report, yes.
